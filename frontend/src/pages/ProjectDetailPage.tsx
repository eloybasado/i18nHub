...
import type { AnalysisReport, IngestResponse, IssueType, Language, Project, RunAnalysisResponse } from '../lib/types';
...
export function ProjectDetailPage() {
  ...
  const onIngest = async () => {
    ...
    try {
      ...
      setIngestMessage(`Se cargaron ${response.filesIngested} archivo(s) en ${response.fileGroupsAffected} grupo(s)`);
    } catch {
  ...
  return (
    <main className="app-shell">
              ...
              return (
                <li key={language.id}>
                  ...
                  {isReference ? (
                    ...
                    <button type="button" className="ghost" onClick={() => setReference(language.id)}>
                      <Star size={14} />
                      Marcar referencia
                    </button>
                  )}
      ...
      <section className="panel">
        ...
        <p className="muted">
          Patron actual: <strong>{project ? PATTERN_LABELS[project.i18nPattern] : '-'}</strong>
        </p>
      ...
      <section className="panel">
        ...
        {!project?.referenceLanguageId ? (
          <p className="error" style={{ marginTop: '10px' }}>
            <AlertTriangle size={14} style={{ verticalAlign: 'text-bottom' }} /> Debes marcar un idioma de referencia
